...
namespace FBApiApplication.CS
{
    public class Service : IDisposable
    {
                    ...
                    if (webResponse.StatusDescription == "OK")
                    {
                        using (System.IO.Stream s = webrequest.GetResponse().GetResponseStream())
                        {
                            using (System.IO.StreamReader sr = new System.IO.StreamReader(s))
                            {
                                ...
                                var respnsData = JsonConvert.DeserializeObject(jsonResponse);

                                ResponseData Successdata = JsonConvert.DeserializeObject<ResponseData>(respnsData.ToString());
                                if (Successdata.Status == "Success")
                                {
                                    UpdateTrace(Successdata.Bank_TrnId);
                                }
                                else
                                {
                                    //write log with full json string
        ...
        //Backup_Data_Log
        public void InsertBackupLogData()
        {
            try
            {
                DbCommand command = _dbContext.LiveExecuteStoredProcesure("wsp_Ins_cus_depositor_trn_log");
                string message = "";
                try
                {
   ...
   //InsertTranseData
        public void InsertTraceData()
        {
            DbCommand command = _dbContext.LiveExecuteStoredProcesure("wsp_Ins_cus_depositor_trn_Auto");
            string message = "";
            try
            {
