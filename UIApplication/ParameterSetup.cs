    ...
    public partial class ParameterSetup : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (CBSYN == "Y")
            {
                ...
                {
                    messages=objSendData.Save_Parameter(Int32.Parse(txtSendAft.Text), Int32.Parse(txtReloadAft.Text), txtApiUrl.Text, txtApiUser.Text, txtPassword.Text,rdYes.Text);
                    if (messages[1] == "OK")
                    {
                        Settings.WebApiUrl = txtApiUrl.Text.ToString();
                        Settings.Apiusername = txtApiUser.Text.ToString();
                        Settings.Apiuserpass = txtPassword.Text.ToString();
                ...
                {
                    messages=objSendData.Save_Parameter(Int32.Parse(txtSendAft.Text), Int32.Parse(txtReloadAft.Text), txtApiUrl.Text, txtApiUser.Text, txtPassword.Text, rdNo.Text);
                    if (messages[1] == "OK")
                    {
                        Settings.WebApiUrl = txtApiUrl.Text.ToString();
                        Settings.Apiusername = txtApiUser.Text.ToString();
        ...
        void Parameter_Reload()
        {
            try
            {
                ...
                if(pdata != null && pdata.SendAft.Length>0)
                {
                    ...
                    txtPassword.Text = pdata.ApiPass.ToString();
